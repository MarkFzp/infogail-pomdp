...
class Discriminator:
    def __init__(self, config, sess):
        ...
        with tf.variable_scope('Discriminator'):
            self.expert_state = tf.placeholder(tf.float32, [None, None, config.state_dim])
            self.expert_action = tf.placeholder(tf.float32, [None, None, config.action_dim])
            ...
            self.param = [w for w in tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope = 'Discriminator/') if 'bias' not in w.name]
            print(self.param)
    ...
    def _get_score(self, expert_state, expert_action, stu_state, stu_action):
        ...
        expert_action_layer = expert_action
        stu_action_layer = stu_action
        ...
        with tf.variable_scope('mlp'):
            for dim in self.config.dis_fc_dims:
                dense = tf.layers.Dense(dim, activation = self.config.activation, kernel_initializer = init, name = 'mlp')
