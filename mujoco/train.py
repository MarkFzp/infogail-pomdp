...
def get_env():
    if config.n_cpu == 1:
        env = DummyVecEnv([lambda: gym.make('Hopper-v3', forward_reward_weight = 1.0, \
            ctrl_cost_weight = 1e-3, terminate_when_unhealthy = False)])
    else:
        env = SubprocVecEnv([lambda: gym.make('Hopper-v3', forward_reward_weight = 1.0, \
            ctrl_cost_weight = 1e-3, terminate_when_unhealthy = False) for i in range(config.n_cpu)])
    
    return env


def load_expert_traj():
    traj_np = np.concatenate(
        [np.load('{}_{}_state.npy'.format(config.expert_traj_prefix, suffix))[:, :config.max_traj_len, :config.state_dim] for suffix in ['forward', 'stand', 'backward']], 
        axis = 0
    )
    theta_np = np.concatenate(
        [np.load('{}_{}_action.npy'.format(config.expert_traj_prefix, suffix))[:, :config.max_traj_len, :] for suffix in ['forward', 'stand', 'backward']], 
        axis = 0
    )
    print()
...
def code_reward_helper(stu_traj_code, f_r_np, s_r_np, b_r_np, code_1_f_r, code_1_s_r, code_1_b_r, code_2_f_r, code_2_s_r, code_2_b_r, code_3_f_r,
    code_3_s_r, code_3_b_r):

    code_1_idx = np.where(stu_traj_code == 0)[0]
    if len(code_1_idx) > 0:
        code_1_f_r.extend(f_r_np[code_1_idx])
        code_1_s_r.extend(s_r_np[code_1_idx])
        code_1_b_r.extend(b_r_np[code_1_idx])
    
    code_2_idx = np.where(stu_traj_code == 1)[0]
    if len(code_2_idx) > 0:
        code_2_f_r.extend(f_r_np[code_2_idx])
        code_2_s_r.extend(s_r_np[code_2_idx])
        code_2_b_r.extend(b_r_np[code_2_idx])
    
    code_3_idx = np.where(stu_traj_code == 2)[0]
    if len(code_3_idx) > 0:
        code_3_f_r.extend(f_r_np[code_3_idx])
        code_3_s_r.extend(s_r_np[code_3_idx])
        code_3_b_r.extend(b_r_np[code_3_idx])
...
def main():
    ...
    if config.mode == 'test':
        for i in tqdm(range(config.test_itr)):
            stu_traj_state, stu_traj_action, stu_traj_code, f_r_np, s_r_np, b_r_np = buffer.sample_stu_traj()
            code_reward_helper(stu_traj_code, f_r_np, s_r_np, b_r_np, code_1_f_r, code_1_s_r, code_1_b_r, code_2_f_r, code_2_s_r, code_2_b_r, 
                code_3_f_r, code_3_s_r, code_3_b_r)
        
        print_and_clear(code_1_reward, code_2_reward, code_3_reward)
        ...
        for i in range(1, config.itr + 1):
            ...
            if i % config.print_itr == 0:
                print('[t{}]'.format(i))
                print_and_clear(dis_stat, post_stat, gae_stat, policy_stat, value_stat, code_1_reward, code_2_reward, code_3_reward)
