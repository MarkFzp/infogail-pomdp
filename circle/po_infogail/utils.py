...
def log_gaussian_prob_density(mean: "2d tensor", log_std: "scalar tensor", x: "2d tensor"):
...
def normalize(x: "2d tensor"):
    ...
    std = (tf.reduce_sum((x - mean) ** 2) / (tf.to_float(tf.reduce_prod(tf.shape(x))) - 1.0)) ** 0.5
