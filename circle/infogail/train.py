...
def bc_pretrain():
    ...
    for i in range(1, config.itr + 1):
        expert_state, expert_action = buffer.sample_expert_exp()
        expert_code = buffer.sample_code()
        loss = policy.bc_train(expert_state, expert_code, expert_action)
        losses.append(loss)
...
def gae(stu_traj_state, stu_traj_action, stu_traj_code, stu_exp_idx):
    next_value = None
    value_spvs_reversed = []
    adv_reversed = []
    ...
    for t in reversed(range(config.max_traj_len)):
        curr_state = stu_traj_state[:, t]
        curr_action = stu_traj_action[:, t]
        curr_value = value_net.get_value(curr_state, stu_traj_code)
        
        dis_score = discriminator.get_stu_score(curr_state, curr_action)
        log_p = posterior.get_log_posterior(curr_state, curr_action, stu_traj_code)
        dis_scores.append(dis_score)
        log_ps.append(log_p)

        r = dis_score + config.post_coef * log_p

        if t == config.max_traj_len - 1:
            value_spvs = r
            adv = r - curr_value
        else:
            td = r + config.gamma * next_value - curr_value
            value_spvs = r + config.gamma * (config.lam * value_spvs_reversed[-1] + (1 - config.lam) * next_value)
    ...
    exp_value_spvs = value_spvs[stu_exp_idx]
    exp_adv = adv[stu_exp_idx]
    dis_score_mean = np.mean(dis_scores)
    log_p_mean = np.mean(log_ps)

    return exp_value_spvs, exp_adv, dis_score_mean, log_p_mean
...
def main():
    ...
    def warm_up():
        for i in range(1, config.warm_up_itr + 1):

            for j in range(config.inner_itr_1):
                stu_traj_state, stu_traj_action, stu_traj_code = buffer.sample_stu_traj()
                stu_exp_state, stu_exp_action, stu_exp_code, _ = buffer.sample_stu_exp(stu_traj_state, stu_traj_action, stu_traj_code)
                expert_state, expert_action = buffer.sample_expert_exp()
                
                dis_loss, dis_expert_score, dis_stu_score = discriminator.train(expert_state, expert_action, stu_exp_state, stu_exp_action)
                post_loss = posterior.train(stu_exp_state, stu_exp_action, stu_exp_code)
                ...
                if j == config.inner_itr_1 - 1:
                    ...
                    stat_collect(post_losses, post_loss)
            
            for k in range(config.inner_itr_2):
                stu_traj_state, stu_traj_action, stu_traj_code = buffer.sample_stu_traj()
                stu_exp_state, stu_exp_action, stu_exp_code, stu_exp_idx = buffer.sample_stu_exp(stu_traj_state, stu_traj_action, stu_traj_code)

                stu_exp_value_spvs, stu_exp_adv, dis_score, log_p = gae(stu_traj_state, stu_traj_action, stu_traj_code, stu_exp_idx)
                stu_exp_value_old = old_value_net.get_value(stu_exp_state, stu_exp_code)

                value_loss, value, value_clipped_freq = value_net.train(stu_exp_state, stu_exp_code, stu_exp_value_spvs, stu_exp_value_old)
                
                if k == config.inner_itr_2 - 1:
                    stat_collect(
                        dis_scores, dis_score, 
                        log_ps, log_p, 
                        stu_values, stu_exp_value_spvs, 
                        stu_advs, stu_exp_adv, 
                        value_losses, value_loss,
                        values, value,
                        value_clipped_freqs, value_clipped_freq,
                        old_values, stu_exp_value_old
                    )

            if i % config.print_itr == 0:
                print('[t{}]'.format(i))
                print_and_clear(dis_stat, post_stat, gae_stat, value_stat)

                file_name = os.path.join(config.draw_traj_dir, ckpt_name, '{}_warmup_{}.{}'.format(ckpt_name, i, config.draw_extension))
                env.draw_traj_theta(stu_traj_action, stu_traj_code, file_name)
    ...
    for i in range(1, config.itr + 1):
        ...
        for k in range(config.inner_itr_2):
            ...
            stu_exp_value_spvs, stu_exp_adv, dis_score, log_p = gae(stu_traj_state, stu_traj_action, stu_traj_code, stu_exp_idx)
            action_log_prob_old = old_policy.get_action_log_prob(stu_exp_state, stu_exp_code, stu_exp_action)
            stu_exp_value_old = old_value_net.get_value(stu_exp_state, stu_exp_code)

            policy_loss, policy_reward, entropy, policy_clipped_freq = policy.train(stu_exp_state, stu_exp_code, stu_exp_action, 
                stu_exp_adv, action_log_prob_old)
            value_loss, value, value_clipped_freq = value_net.train(stu_exp_state, stu_exp_code, stu_exp_value_spvs, stu_exp_value_old)
        ...
        if i % config.save_itr == 0:
            saver.save(sess, config.save_path, global_step = i)
            print('ckpt saved\n')
